...
local _ENV = TEST_CASE "test x509_ca"
if not crypto.x509_ca then test = FAIL"crypto.x509_ca is unavaliable" else

local ca

function setup()
  ca = assert_userdata(crypto.x509_ca())
  assert_true(ca:add_pem(ca_cert))
end
...
function test_invalid_cert()
  assert_nil(ca:add_pem("FOBAR"), "bad cert succeeded")
end

function test_verify()
  assert_true(ca:verify_pem(cert_to_verify), "failed to verify good cert")
end

function test_fail()
  assert_false(ca:verify_pem(bad_cert), "verified bad cert, noooooo")
end
